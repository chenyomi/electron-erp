...
function getDataDir() {
  const dir = path.join(electron.app.getPath("userData"), "ledger-data");
...
function logOperation(tableName, recordId, action, oldData, newData, operator = "") {
  const db2 = getDb();
  db2.prepare(`
...
function softDelete(tableName, id) {
  const db2 = getDb();
  const row = db2.prepare(`SELECT * FROM ${tableName} WHERE id = ?`).get(id);
  if (!row) return;
  db2.prepare(`UPDATE ${tableName} SET deleted_at = datetime('now','localtime') WHERE id = ?`).run(id);
  logOperation(tableName, id, "DELETE", row, null);
}
function restore(tableName, id) {
  const db2 = getDb();
  const row = db2.prepare(`SELECT * FROM ${tableName} WHERE id = ?`).get(id);
  if (!row) return;
  db2.prepare(`UPDATE ${tableName} SET deleted_at = NULL WHERE id = ?`).run(id);
  logOperation(tableName, id, "RESTORE", null, row);
}
function registerCashHandlers() {
  electron.ipcMain.handle("cash:list", (_e, { page = 1, pageSize = 50, keyword = "" } = {}) => {
    const db2 = getDb();
  ...
  electron.ipcMain.handle("cash:summary", () => {
    const db2 = getDb();
    return db2.prepare(`
      SELECT
  ...
  electron.ipcMain.handle("cash:monthly", () => {
    const db2 = getDb();
    return db2.prepare(`
      SELECT
  ...
  electron.ipcMain.handle("cash:add", (_e, row) => {
    const db2 = getDb();
    ...
    const newRow = db2.prepare("SELECT * FROM cash_ledger WHERE id = ?").get(result.lastInsertRowid);
    logOperation("cash_ledger", result.lastInsertRowid, "INSERT", null, newRow);
    return newRow;
  });
  electron.ipcMain.handle("cash:update", (_e, { id, ...row }) => {
    const db2 = getDb();
    const oldRow = db2.prepare("SELECT * FROM cash_ledger WHERE id = ?").get(id);
    db2.prepare(`
      UPDATE cash_ledger
      ...
      WHERE id=@id
    `).run({ ...row, id });
    const newRow = db2.prepare("SELECT * FROM cash_ledger WHERE id = ?").get(id);
    logOperation("cash_ledger", id, "UPDATE", oldRow, newRow);
    return newRow;
  });
  electron.ipcMain.handle("cash:delete", (_e, id) => {
    softDelete("cash_ledger", id);
    return { ok: true };
  });
  electron.ipcMain.handle("cash:trash", () => {
    const db2 = getDb();
    return db2.prepare(`SELECT * FROM cash_ledger WHERE deleted_at IS NOT NULL ORDER BY deleted_at DESC`).all();
  });
  electron.ipcMain.handle("cash:restore", (_e, id) => {
    restore("cash_ledger", id);
    return { ok: true };
  });
}
function registerBankHandlers() {
  electron.ipcMain.handle("bank:list", (_e, { page = 1, pageSize = 50, keyword = "" } = {}) => {
    const db2 = getDb();
    const offset = (page - 1) * pageSize;
    const like = `%${keyword}%`;
    const rows = db2.prepare(`
  ...
  electron.ipcMain.handle("bank:summary", () => {
    const db2 = getDb();
    return db2.prepare(`
  ...
  electron.ipcMain.handle("bank:monthly", () => {
    const db2 = getDb();
    return db2.prepare(`
  ...
  electron.ipcMain.handle("bank:add", (_e, row) => {
    const db2 = getDb();
    const r = db2.prepare(`
    ...
    `).run(row);
    const newRow = db2.prepare("SELECT * FROM bank_ledger WHERE id = ?").get(r.lastInsertRowid);
    logOperation("bank_ledger", r.lastInsertRowid, "INSERT", null, newRow);
    return newRow;
  });
  electron.ipcMain.handle("bank:update", (_e, { id, ...row }) => {
    const db2 = getDb();
    const old = db2.prepare("SELECT * FROM bank_ledger WHERE id = ?").get(id);
    db2.prepare(`
    ...
    `).run({ ...row, id });
    const newRow = db2.prepare("SELECT * FROM bank_ledger WHERE id = ?").get(id);
    logOperation("bank_ledger", id, "UPDATE", old, newRow);
    return newRow;
  });
  electron.ipcMain.handle("bank:delete", (_e, id) => {
    softDelete("bank_ledger", id);
    return { ok: true };
  });
  electron.ipcMain.handle("bank:trash", () => {
    return getDb().prepare(`SELECT * FROM bank_ledger WHERE deleted_at IS NOT NULL ORDER BY deleted_at DESC`).all();
  });
  electron.ipcMain.handle("bank:restore", (_e, id) => {
    restore("bank_ledger", id);
    return { ok: true };
  });
}
function registerBillsHandlers() {
  electron.ipcMain.handle("bills:list", (_e, { page = 1, pageSize = 50, keyword = "" } = {}) => {
    const db2 = getDb();
    const offset = (page - 1) * pageSize;
    const like = `%${keyword}%`;
    const rows = db2.prepare(`
      SELECT * FROM acceptance_bills
      WHERE deleted_at IS NULL AND (description LIKE ? OR note LIKE ? OR date LIKE ?)
      ORDER BY date ASC, id ASC LIMIT ? OFFSET ?
    `).all(like, like, like, pageSize, offset);
    const { total } = db2.prepare(`
  ...
  electron.ipcMain.handle("bills:summary", () => {
    const db2 = getDb();
    return db2.prepare(`
      SELECT SUM(amount_in) as totalIn, SUM(amount_out) as totalOut
  ...
  electron.ipcMain.handle("bills:add", (_e, row) => {
    const db2 = getDb();
    const r = db2.prepare(`
      INSERT INTO acceptance_bills (date, description, amount_in, amount_out, balance, note)
      VALUES (@date, @description, @amount_in, @amount_out, @balance, @note)
    `).run(row);
    const newRow = db2.prepare("SELECT * FROM acceptance_bills WHERE id = ?").get(r.lastInsertRowid);
    logOperation("acceptance_bills", r.lastInsertRowid, "INSERT", null, newRow);
    return newRow;
  });
  electron.ipcMain.handle("bills:update", (_e, { id, ...row }) => {
    const db2 = getDb();
    const old = db2.prepare("SELECT * FROM acceptance_bills WHERE id = ?").get(id);
    db2.prepare(`
      UPDATE acceptance_bills SET date=@date, description=@description,
        amount_in=@amount_in, amount_out=@amount_out, balance=@balance,
        note=@note, updated_at=datetime('now','localtime') WHERE id=@id
    `).run({ ...row, id });
    const newRow = db2.prepare("SELECT * FROM acceptance_bills WHERE id = ?").get(id);
    logOperation("acceptance_bills", id, "UPDATE", old, newRow);
    return newRow;
  });
  electron.ipcMain.handle("bills:delete", (_e, id) => {
    softDelete("acceptance_bills", id);
    return { ok: true };
  });
  electron.ipcMain.handle("bills:trash", () => {
    return getDb().prepare(`SELECT * FROM acceptance_bills WHERE deleted_at IS NOT NULL ORDER BY deleted_at DESC`).all();
  });
  electron.ipcMain.handle("bills:restore", (_e, id) => {
    restore("acceptance_bills", id);
    return { ok: true };
  });
}
function registerCustomerHandlers() {
  electron.ipcMain.handle("customer:names", () => {
    const db2 = getDb();
    return db2.prepare(`SELECT DISTINCT customer_name FROM customer_ledger WHERE deleted_at IS NULL ORDER BY customer_name`).all();
  });
  electron.ipcMain.handle("customer:list", (_e, { customerName, page = 1, pageSize = 100, keyword = "" } = {}) => {
    const db2 = getDb();
    const offset = (page - 1) * pageSize;
    const like = `%${keyword}%`;
    const rows = db2.prepare(`
      ...
      LIMIT ? OFFSET ?
    `).all(customerName || "", customerName || "", like, like, like, pageSize, offset);
    const { total } = db2.prepare(`
      SELECT COUNT(*) as total FROM customer_ledger
      WHERE deleted_at IS NULL
        AND (? = '' OR customer_name = ?)
        AND (description LIKE ? OR note LIKE ? OR date LIKE ?)
    `).get(customerName || "", customerName || "", like, like, like);
    return { rows, total };
  });
  electron.ipcMain.handle("customer:summary", (_e, customerName = "") => {
    const db2 = getDb();
    if (customerName) {
      return db2.prepare(`
  ...
  electron.ipcMain.handle("customer:add", (_e, row) => {
    const db2 = getDb();
    const r = db2.prepare(`
    ...
    `).run(row);
    const newRow = db2.prepare("SELECT * FROM customer_ledger WHERE id = ?").get(r.lastInsertRowid);
    logOperation("customer_ledger", r.lastInsertRowid, "INSERT", null, newRow);
    return newRow;
  });
  electron.ipcMain.handle("customer:update", (_e, { id, ...row }) => {
    const db2 = getDb();
    const old = db2.prepare("SELECT * FROM customer_ledger WHERE id = ?").get(id);
    db2.prepare(`
    ...
    `).run({ ...row, id });
    const newRow = db2.prepare("SELECT * FROM customer_ledger WHERE id = ?").get(id);
    logOperation("customer_ledger", id, "UPDATE", old, newRow);
    return newRow;
  });
  electron.ipcMain.handle("customer:delete", (_e, id) => {
    softDelete("customer_ledger", id);
    return { ok: true };
  });
  electron.ipcMain.handle("customer:trash", () => {
    return getDb().prepare(`SELECT * FROM customer_ledger WHERE deleted_at IS NOT NULL ORDER BY deleted_at DESC`).all();
  });
  electron.ipcMain.handle("customer:restore", (_e, id) => {
...
function autoBackup() {
  try {
    ...
    if (!fs__namespace.existsSync(backupDir)) fs__namespace.mkdirSync(backupDir, { recursive: true });
    const now = /* @__PURE__ */ new Date();
    const ts = now.toISOString().replace(/[:.]/g, "-").slice(0, 19);
    const backupPath = path.join(backupDir, `ledger_${ts}.db`);
    const db2 = getDb();
    db2.backup(backupPath);
    const files = fs__namespace.readdirSync(backupDir).filter((f) => f.endsWith(".db")).map((f) => ({ name: f, time: fs__namespace.statSync(path.join(backupDir, f)).mtime.getTime() })).sort((a, b) => b.time - a.time);
    if (files.length > 30) {
      files.slice(30).forEach((f) => {
        fs__namespace.unlinkSync(path.join(backupDir, f.name));
      });
    }
  } catch (e) {
...
function listBackups() {
  ...
  return fs__namespace.readdirSync(backupDir).filter((f) => f.endsWith(".db")).map((f) => {
    const stat = fs__namespace.statSync(path.join(backupDir, f));
    return { name: f, time: stat.mtime.toISOString(), size: stat.size };
  }).sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime());
}
function registerSystemHandlers() {
  electron.ipcMain.handle("system:summary", () => {
    const db2 = getDb();
    const cash = db2.prepare(`SELECT SUM(income) as totalIncome, SUM(expense) as totalExpense FROM cash_ledger WHERE deleted_at IS NULL`).get();
    const bank = db2.prepare(`SELECT SUM(amount_in) as totalIn, SUM(amount_out) as totalOut FROM bank_ledger WHERE deleted_at IS NULL`).get();
    const bills = db2.prepare(`SELECT SUM(amount_in) as totalIn, SUM(amount_out) as totalOut FROM acceptance_bills WHERE deleted_at IS NULL`).get();
    db2.prepare(`SELECT balance FROM cash_ledger WHERE deleted_at IS NULL ORDER BY date ASC, id ASC LIMIT 1`).get();
    const cashLastBalance = db2.prepare(`SELECT balance FROM cash_ledger WHERE deleted_at IS NULL ORDER BY date DESC, id DESC LIMIT 1`).get();
    return {
      cash: {
  ...
  electron.ipcMain.handle("system:logs", (_e, { page = 1, pageSize = 50 } = {}) => {
    const db2 = getDb();
    const offset = (page - 1) * pageSize;
    const rows = db2.prepare(`SELECT * FROM operation_logs ORDER BY created_at DESC LIMIT ? OFFSET ?`).all(pageSize, offset);
    const { total } = db2.prepare(`SELECT COUNT(*) as total FROM operation_logs`).get();
    return { rows, total };
  });
  electron.ipcMain.handle("system:trash-all", () => {
    const db2 = getDb();
    const tables = ["cash_ledger", "bank_ledger", "acceptance_bills", "customer_ledger"];
    const result = {};
    for (const t of tables) {
      result[t] = db2.prepare(`SELECT * FROM ${t} WHERE deleted_at IS NOT NULL ORDER BY deleted_at DESC`).all();
    }
    return result;
  });
  electron.ipcMain.handle("system:backup", () => {
    autoBackup();
    return { ok: true };
  });
  electron.ipcMain.handle("system:backups-list", () => {
    return listBackups();
  });
  electron.ipcMain.handle("system:data-dir", () => {
    return getDataDir();
  });
  electron.ipcMain.handle("system:monthly-all", () => {
    const db2 = getDb();
    const cash = db2.prepare(`
      SELECT substr(date,1,7) as month, SUM(income) as income, SUM(expense) as expense
      FROM cash_ledger WHERE deleted_at IS NULL
      GROUP BY substr(date,1,7) ORDER BY month ASC
    `).all();
    const bank = db2.prepare(`
      SELECT substr(date,1,7) as month, SUM(amount_in) as income, SUM(amount_out) as expense
      FROM bank_ledger WHERE deleted_at IS NULL
      GROUP BY substr(date,1,7) ORDER BY month ASC
    `).all();
...
function registerImportHandlers() {
  electron.ipcMain.handle("import:pick-file", async () => {
  ...
  electron.ipcMain.handle("import:excel", (_e, filePath) => {
    try {
      const db2 = getDb();
      const wb = readFileSync(filePath);
      let imported = { cash: 0, bank: 0, bills: 0, customers: 0 };
      const mainSheet = wb.Sheets["帐本"] || wb.Sheets[wb.SheetNames[0]];
      if (mainSheet) {
        const rows = utils.sheet_to_json(mainSheet, { header: 1, defval: "" });
        importMainSheet(db2, rows, imported);
      }
      const skipSheets = /* @__PURE__ */ new Set(["帐本", "Sheet2", "Sheet5", "Sheet6", "滨海昊亮备份"]);
      for (const name of wb.SheetNames) {
        if (skipSheets.has(name)) continue;
        const sheet = wb.Sheets[name];
        if (!sheet) continue;
        const rows = utils.sheet_to_json(sheet, { header: 1, defval: "" });
        importCustomerSheet(db2, name, rows, imported);
      }
...
function importMainSheet(db2, rows, imported) {
  const insertCash = db2.prepare(`INSERT OR IGNORE INTO cash_ledger (date,income,description,expense,operator,balance,note) VALUES (?,?,?,?,?,?,?)`);
  const insertBank = db2.prepare(`INSERT OR IGNORE INTO bank_ledger (date,description,amount_in,amount_out,balance,note) VALUES (?,?,?,?,?,?)`);
  const insertBills = db2.prepare(`INSERT OR IGNORE INTO acceptance_bills (date,description,amount_in,amount_out,balance,note) VALUES (?,?,?,?,?,?)`);
  const insertMany = db2.transaction((data) => {
    for (const r of data) {
      const [c, b, bills] = r;
      if (c) insertCash.run(...c);
      if (b) insertBank.run(...b);
      if (bills) insertBills.run(...bills);
    }
  });
  const batch = [];
  for (let i = 3; i < rows.length; i++) {
    const row = rows[i];
    const dateA = strVal(row[0]);
    const dateH = strVal(row[7]);
    const dateN = strVal(row[13]);
    ...
    batch.push([cashRow, bankRow, billsRow]);
  }
  insertMany(batch);
}
function importCustomerSheet(db2, sheetName, rows, imported) {
  ...
  for (let i = 3; i < rows.length; i++) {
    const row = rows[i];
    const date = strVal(row[0]);
    if (!date || date.includes("合计") || date.includes("月合计")) continue;
    const desc = strVal(row[1]);
    const amtIn = numVal(row[2]);
    const amtOut = numVal(row[3]);
    const balance = numVal(row[4]);
    const note = strVal(row[5]);
    if (date || desc || amtIn || amtOut) {
      batch.push([sheetName, date, desc, amtIn, amtOut, balance, note, ""]);
      imported.customers++;
    }
  }
  insertMany(batch);
