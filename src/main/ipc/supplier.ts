...
export function registerSupplierHandlers(): void {
  ...
  ipcMain.handle('supplier:add', (_e, row) => {
    ...
    if (isPayment) {
      ...
      if (refId > 0) {
        ...
        if (!ref || ref.supplier_name !== supplierName || !isSupplierPayableRecord(ref)) {
          throw new Error('关联的应付记录无效')
        }
    ...
    const r = db.prepare(`
      INSERT INTO supplier_ledger (
        ...
        amount_in, amount_out, balance, note, stock_in_id, ref_ledger_id
      ) VALUES (
        @supplier_name, @date, @description, @contract_no, @product_name, @spec, @unit, @quantity, @unit_price,
        @amount_in, @amount_out, @balance, @note, @stock_in_id, @ref_ledger_id
      )
  ...
  ipcMain.handle('supplier:update', (_e, { id, ...row }) => {
    ...
    if (Number(old.stock_in_id || 0) > 0) {
      throw new Error('历史入库关联的应付请直接在供应商往来中修改或删除')
    }

    const isPayment = isSupplierPaymentRecord(row) || isSupplierPaymentRecord(old)
    const payload = {
      id,
    ...
    if (isPayment) {
      ...
      if (refId > 0) {
        const ref = db.prepare('SELECT * FROM supplier_ledger WHERE id = ? AND deleted_at IS NULL').get(refId) as Record<string, any> | undefined
        if (!ref || ref.supplier_name !== payload.supplier_name || !isSupplierPayableRecord(ref)) {
          throw new Error('关联的应付记录无效')
        }
        payload.ref_ledger_id = refId
      }
    ...
    const newRow = db.prepare('SELECT * FROM supplier_ledger WHERE id = ?').get(id)
